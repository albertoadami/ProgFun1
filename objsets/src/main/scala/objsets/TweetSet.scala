...
class Tweet(val user: String, val text: String, val retweets: Int) {
  ...
  def >=(that: Tweet): Boolean = if (retweets >= that.retweets) true else false
...
class Empty extends TweetSet {
  ...
  def mostRetweeted = throw new NoSuchElementException("No tweets")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    newAcc.union(left.filterAcc(p, acc).union(right.filterAcc(p, acc)))
  }

  def union(that: TweetSet): _root_.objsets.TweetSet =
  ...
  //empty right & left
    if (left.isEmpty && right.isEmpty)
      elem
    //empty right --> search in left
    else if(right.isEmpty)
      max(elem, left.mostRetweeted)
    //empty left --> search in right
    else if(left.isEmpty)
      max(elem, right.mostRetweeted)
    else
    //left & right not empty
      max(elem, max(left.mostRetweeted, right.mostRetweeted))

  private def max(first: Tweet, second: Tweet): Tweet = if (first >= second) first else second
